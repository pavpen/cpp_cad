...
void log_polygon_extrusion_builder_vertices(
    const cpp_cad::Polygon_2 &polygon, double height,
    ...
    std::string extra_code = "")
{
    ...
    {
        extra_code_buf << R"code(
    geometry.vertices.push(new THREE.Vector3()code" <<
        vit->x() << ", " <<
        vit->y() << ", " <<
        (CGAL::to_double(vit->z()) + height) << "));" << std::endl;
    }
    ...
    {
        extra_code_buf << R"code(
    geometry.vertices.push(new THREE.Vector3()code" << vit->x() << ", " << vit->y() << ", " << vit->z() << "));" << std::endl;
    }
...
template <class HDS>
inline void log_polygon_extrusion_builder_vertices(
